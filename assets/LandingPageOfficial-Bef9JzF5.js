import{r as pe,c as Te,g as Oe,j as l,m as Ce}from"./DynamicThemeProvider-CxehgrR0.js";import{c as u,a as ae}from"./emotion-css.esm-CiHGz3gB.js";import{R as G,g as Ie,a as Ue,b as Pe,c as Me,f as Re,d as We}from"./RetinaImage-C1tj6ORK.js";import{S as Ye}from"./logo-HHJTHOAP.js";import{a as ke,c as se,d as ce}from"./Button-Dt6jELX4.js";import{G as Ge,a as Je}from"./GitRevision-BywqO7Dn.js";import{u as Le}from"./useDocumentTitle-D3L4F_h2.js";var ye={exports:{}};(function(K,L){(function(D,te){K.exports=te(pe)})(typeof self<"u"?self:Te,D=>(()=>{var te={7403:(t,o,e)=>{e.d(o,{default:()=>q});var r=e(4087),a=e.n(r);const i=function(B){return new RegExp(/<[a-z][\s\S]*>/i).test(B)},n=function(B,A){return Math.floor(Math.random()*(A-B+1))+B};var c="TYPE_CHARACTER",p="REMOVE_CHARACTER",d="REMOVE_ALL",m="REMOVE_LAST_VISIBLE_NODE",x="PAUSE_FOR",g="CALL_FUNCTION",v="ADD_HTML_TAG_ELEMENT",h="CHANGE_DELETE_SPEED",b="CHANGE_DELAY",j="CHANGE_CURSOR",F="PASTE_STRING",O="HTML_TAG";function N(B){return N=typeof Symbol=="function"&&typeof Symbol.iterator=="symbol"?function(A){return typeof A}:function(A){return A&&typeof Symbol=="function"&&A.constructor===Symbol&&A!==Symbol.prototype?"symbol":typeof A},N(B)}function y(B,A){var T=Object.keys(B);if(Object.getOwnPropertySymbols){var S=Object.getOwnPropertySymbols(B);A&&(S=S.filter(function(z){return Object.getOwnPropertyDescriptor(B,z).enumerable})),T.push.apply(T,S)}return T}function E(B){for(var A=1;A<arguments.length;A++){var T=arguments[A]!=null?arguments[A]:{};A%2?y(Object(T),!0).forEach(function(S){C(B,S,T[S])}):Object.getOwnPropertyDescriptors?Object.defineProperties(B,Object.getOwnPropertyDescriptors(T)):y(Object(T)).forEach(function(S){Object.defineProperty(B,S,Object.getOwnPropertyDescriptor(T,S))})}return B}function I(B){return function(A){if(Array.isArray(A))return U(A)}(B)||function(A){if(typeof Symbol<"u"&&A[Symbol.iterator]!=null||A["@@iterator"]!=null)return Array.from(A)}(B)||function(A,T){if(A){if(typeof A=="string")return U(A,T);var S=Object.prototype.toString.call(A).slice(8,-1);return S==="Object"&&A.constructor&&(S=A.constructor.name),S==="Map"||S==="Set"?Array.from(A):S==="Arguments"||/^(?:Ui|I)nt(?:8|16|32)(?:Clamped)?Array$/.test(S)?U(A,T):void 0}}(B)||function(){throw new TypeError(`Invalid attempt to spread non-iterable instance.
In order to be iterable, non-array objects must have a [Symbol.iterator]() method.`)}()}function U(B,A){(A==null||A>B.length)&&(A=B.length);for(var T=0,S=new Array(A);T<A;T++)S[T]=B[T];return S}function P(B,A){for(var T=0;T<A.length;T++){var S=A[T];S.enumerable=S.enumerable||!1,S.configurable=!0,"value"in S&&(S.writable=!0),Object.defineProperty(B,V(S.key),S)}}function C(B,A,T){return(A=V(A))in B?Object.defineProperty(B,A,{value:T,enumerable:!0,configurable:!0,writable:!0}):B[A]=T,B}function V(B){var A=function(T,S){if(N(T)!=="object"||T===null)return T;var z=T[Symbol.toPrimitive];if(z!==void 0){var s=z.call(T,"string");if(N(s)!=="object")return s;throw new TypeError("@@toPrimitive must return a primitive value.")}return String(T)}(B);return N(A)==="symbol"?A:String(A)}const q=function(){function B(S,z){var s=this;if(function(w,M){if(!(w instanceof M))throw new TypeError("Cannot call a class as a function")}(this,B),C(this,"state",{cursorAnimation:null,lastFrameTime:null,pauseUntil:null,eventQueue:[],eventLoop:null,eventLoopPaused:!1,reverseCalledEvents:[],calledEvents:[],visibleNodes:[],initialOptions:null,elements:{container:null,wrapper:document.createElement("span"),cursor:document.createElement("span")}}),C(this,"options",{strings:null,cursor:"|",delay:"natural",pauseFor:1500,deleteSpeed:"natural",loop:!1,autoStart:!1,devMode:!1,skipAddStyles:!1,wrapperClassName:"Typewriter__wrapper",cursorClassName:"Typewriter__cursor",stringSplitter:null,onCreateTextNode:null,onRemoveNode:null}),C(this,"setupWrapperElement",function(){s.state.elements.container&&(s.state.elements.wrapper.className=s.options.wrapperClassName,s.state.elements.cursor.className=s.options.cursorClassName,s.state.elements.cursor.innerHTML=s.options.cursor,s.state.elements.container.innerHTML="",s.state.elements.container.appendChild(s.state.elements.wrapper),s.state.elements.container.appendChild(s.state.elements.cursor))}),C(this,"start",function(){return s.state.eventLoopPaused=!1,s.runEventLoop(),s}),C(this,"pause",function(){return s.state.eventLoopPaused=!0,s}),C(this,"stop",function(){return s.state.eventLoop&&((0,r.cancel)(s.state.eventLoop),s.state.eventLoop=null),s}),C(this,"pauseFor",function(w){return s.addEventToQueue(x,{ms:w}),s}),C(this,"typeOutAllStrings",function(){return typeof s.options.strings=="string"?(s.typeString(s.options.strings).pauseFor(s.options.pauseFor),s):(s.options.strings.forEach(function(w){s.typeString(w).pauseFor(s.options.pauseFor).deleteAll(s.options.deleteSpeed)}),s)}),C(this,"typeString",function(w){var M=arguments.length>1&&arguments[1]!==void 0?arguments[1]:null;if(i(w))return s.typeOutHTMLString(w,M);if(w){var W=(s.options||{}).stringSplitter,Y=typeof W=="function"?W(w):w.split("");s.typeCharacters(Y,M)}return s}),C(this,"pasteString",function(w){var M=arguments.length>1&&arguments[1]!==void 0?arguments[1]:null;return i(w)?s.typeOutHTMLString(w,M,!0):(w&&s.addEventToQueue(F,{character:w,node:M}),s)}),C(this,"typeOutHTMLString",function(w){var M=arguments.length>1&&arguments[1]!==void 0?arguments[1]:null,W=arguments.length>2?arguments[2]:void 0,Y=function(H){var Z=document.createElement("div");return Z.innerHTML=H,Z.childNodes}(w);if(Y.length>0)for(var R=0;R<Y.length;R++){var Q=Y[R],X=Q.innerHTML;Q&&Q.nodeType!==3?(Q.innerHTML="",s.addEventToQueue(v,{node:Q,parentNode:M}),W?s.pasteString(X,Q):s.typeString(X,Q)):Q.textContent&&(W?s.pasteString(Q.textContent,M):s.typeString(Q.textContent,M))}return s}),C(this,"deleteAll",function(){var w=arguments.length>0&&arguments[0]!==void 0?arguments[0]:"natural";return s.addEventToQueue(d,{speed:w}),s}),C(this,"changeDeleteSpeed",function(w){if(!w)throw new Error("Must provide new delete speed");return s.addEventToQueue(h,{speed:w}),s}),C(this,"changeDelay",function(w){if(!w)throw new Error("Must provide new delay");return s.addEventToQueue(b,{delay:w}),s}),C(this,"changeCursor",function(w){if(!w)throw new Error("Must provide new cursor");return s.addEventToQueue(j,{cursor:w}),s}),C(this,"deleteChars",function(w){if(!w)throw new Error("Must provide amount of characters to delete");for(var M=0;M<w;M++)s.addEventToQueue(p);return s}),C(this,"callFunction",function(w,M){if(!w||typeof w!="function")throw new Error("Callback must be a function");return s.addEventToQueue(g,{cb:w,thisArg:M}),s}),C(this,"typeCharacters",function(w){var M=arguments.length>1&&arguments[1]!==void 0?arguments[1]:null;if(!w||!Array.isArray(w))throw new Error("Characters must be an array");return w.forEach(function(W){s.addEventToQueue(c,{character:W,node:M})}),s}),C(this,"removeCharacters",function(w){if(!w||!Array.isArray(w))throw new Error("Characters must be an array");return w.forEach(function(){s.addEventToQueue(p)}),s}),C(this,"addEventToQueue",function(w,M){var W=arguments.length>2&&arguments[2]!==void 0&&arguments[2];return s.addEventToStateProperty(w,M,W,"eventQueue")}),C(this,"addReverseCalledEvent",function(w,M){var W=arguments.length>2&&arguments[2]!==void 0&&arguments[2];return s.options.loop?s.addEventToStateProperty(w,M,W,"reverseCalledEvents"):s}),C(this,"addEventToStateProperty",function(w,M){var W=arguments.length>2&&arguments[2]!==void 0&&arguments[2],Y=arguments.length>3?arguments[3]:void 0,R={eventName:w,eventArgs:M||{}};return s.state[Y]=W?[R].concat(I(s.state[Y])):[].concat(I(s.state[Y]),[R]),s}),C(this,"runEventLoop",function(){s.state.lastFrameTime||(s.state.lastFrameTime=Date.now());var w=Date.now(),M=w-s.state.lastFrameTime;if(!s.state.eventQueue.length){if(!s.options.loop)return;s.state.eventQueue=I(s.state.calledEvents),s.state.calledEvents=[],s.options=E({},s.state.initialOptions)}if(s.state.eventLoop=a()(s.runEventLoop),!s.state.eventLoopPaused){if(s.state.pauseUntil){if(w<s.state.pauseUntil)return;s.state.pauseUntil=null}var W,Y=I(s.state.eventQueue),R=Y.shift();if(!(M<=(W=R.eventName===m||R.eventName===p?s.options.deleteSpeed==="natural"?n(40,80):s.options.deleteSpeed:s.options.delay==="natural"?n(120,160):s.options.delay))){var Q=R.eventName,X=R.eventArgs;switch(s.logInDevMode({currentEvent:R,state:s.state,delay:W}),Q){case F:case c:var H=X.character,Z=X.node,xe=document.createTextNode(H),_=xe;s.options.onCreateTextNode&&typeof s.options.onCreateTextNode=="function"&&(_=s.options.onCreateTextNode(H,xe)),_&&(Z?Z.appendChild(_):s.state.elements.wrapper.appendChild(_)),s.state.visibleNodes=[].concat(I(s.state.visibleNodes),[{type:"TEXT_NODE",character:H,node:_}]);break;case p:Y.unshift({eventName:m,eventArgs:{removingCharacterNode:!0}});break;case x:var we=R.eventArgs.ms;s.state.pauseUntil=Date.now()+parseInt(we);break;case g:var me=R.eventArgs,be=me.cb,je=me.thisArg;be.call(je,{elements:s.state.elements});break;case v:var ge=R.eventArgs,re=ge.node,ne=ge.parentNode;ne?ne.appendChild(re):s.state.elements.wrapper.appendChild(re),s.state.visibleNodes=[].concat(I(s.state.visibleNodes),[{type:O,node:re,parentNode:ne||s.state.elements.wrapper}]);break;case d:var Ee=s.state.visibleNodes,oe=X.speed,$=[];oe&&$.push({eventName:h,eventArgs:{speed:oe,temp:!0}});for(var he=0,Be=Ee.length;he<Be;he++)$.push({eventName:m,eventArgs:{removingCharacterNode:!1}});oe&&$.push({eventName:h,eventArgs:{speed:s.options.deleteSpeed,temp:!0}}),Y.unshift.apply(Y,$);break;case m:var Se=R.eventArgs.removingCharacterNode;if(s.state.visibleNodes.length){var ie=s.state.visibleNodes.pop(),Fe=ie.type,ee=ie.node,Ne=ie.character;s.options.onRemoveNode&&typeof s.options.onRemoveNode=="function"&&s.options.onRemoveNode({node:ee,character:Ne}),ee&&ee.parentNode.removeChild(ee),Fe===O&&Se&&Y.unshift({eventName:m,eventArgs:{}})}break;case h:s.options.deleteSpeed=R.eventArgs.speed;break;case b:s.options.delay=R.eventArgs.delay;break;case j:s.options.cursor=R.eventArgs.cursor,s.state.elements.cursor.innerHTML=R.eventArgs.cursor}s.options.loop&&(R.eventName===m||R.eventArgs&&R.eventArgs.temp||(s.state.calledEvents=[].concat(I(s.state.calledEvents),[R]))),s.state.eventQueue=Y,s.state.lastFrameTime=w}}}),S)if(typeof S=="string"){var fe=document.querySelector(S);if(!fe)throw new Error("Could not find container element");this.state.elements.container=fe}else this.state.elements.container=S;z&&(this.options=E(E({},this.options),z)),this.state.initialOptions=E({},this.options),this.init()}var A,T;return A=B,(T=[{key:"init",value:function(){var S,z;this.setupWrapperElement(),this.addEventToQueue(j,{cursor:this.options.cursor},!0),this.addEventToQueue(d,null,!0),!window||window.___TYPEWRITER_JS_STYLES_ADDED___||this.options.skipAddStyles||(S=".Typewriter__cursor{-webkit-animation:Typewriter-cursor 1s infinite;animation:Typewriter-cursor 1s infinite;margin-left:1px}@-webkit-keyframes Typewriter-cursor{0%{opacity:0}50%{opacity:1}100%{opacity:0}}@keyframes Typewriter-cursor{0%{opacity:0}50%{opacity:1}100%{opacity:0}}",(z=document.createElement("style")).appendChild(document.createTextNode(S)),document.head.appendChild(z),window.___TYPEWRITER_JS_STYLES_ADDED___=!0),this.options.autoStart===!0&&this.options.strings&&this.typeOutAllStrings().start()}},{key:"logInDevMode",value:function(S){this.options.devMode&&console.log(S)}}])&&P(A.prototype,T),Object.defineProperty(A,"prototype",{writable:!1}),B}()},8552:(t,o,e)=>{var r=e(852)(e(5639),"DataView");t.exports=r},1989:(t,o,e)=>{var r=e(1789),a=e(401),i=e(7667),n=e(1327),c=e(1866);function p(d){var m=-1,x=d==null?0:d.length;for(this.clear();++m<x;){var g=d[m];this.set(g[0],g[1])}}p.prototype.clear=r,p.prototype.delete=a,p.prototype.get=i,p.prototype.has=n,p.prototype.set=c,t.exports=p},8407:(t,o,e)=>{var r=e(7040),a=e(4125),i=e(2117),n=e(7518),c=e(4705);function p(d){var m=-1,x=d==null?0:d.length;for(this.clear();++m<x;){var g=d[m];this.set(g[0],g[1])}}p.prototype.clear=r,p.prototype.delete=a,p.prototype.get=i,p.prototype.has=n,p.prototype.set=c,t.exports=p},7071:(t,o,e)=>{var r=e(852)(e(5639),"Map");t.exports=r},3369:(t,o,e)=>{var r=e(4785),a=e(1285),i=e(6e3),n=e(9916),c=e(5265);function p(d){var m=-1,x=d==null?0:d.length;for(this.clear();++m<x;){var g=d[m];this.set(g[0],g[1])}}p.prototype.clear=r,p.prototype.delete=a,p.prototype.get=i,p.prototype.has=n,p.prototype.set=c,t.exports=p},3818:(t,o,e)=>{var r=e(852)(e(5639),"Promise");t.exports=r},8525:(t,o,e)=>{var r=e(852)(e(5639),"Set");t.exports=r},8668:(t,o,e)=>{var r=e(3369),a=e(619),i=e(2385);function n(c){var p=-1,d=c==null?0:c.length;for(this.__data__=new r;++p<d;)this.add(c[p])}n.prototype.add=n.prototype.push=a,n.prototype.has=i,t.exports=n},6384:(t,o,e)=>{var r=e(8407),a=e(7465),i=e(3779),n=e(7599),c=e(4758),p=e(4309);function d(m){var x=this.__data__=new r(m);this.size=x.size}d.prototype.clear=a,d.prototype.delete=i,d.prototype.get=n,d.prototype.has=c,d.prototype.set=p,t.exports=d},2705:(t,o,e)=>{var r=e(5639).Symbol;t.exports=r},1149:(t,o,e)=>{var r=e(5639).Uint8Array;t.exports=r},577:(t,o,e)=>{var r=e(852)(e(5639),"WeakMap");t.exports=r},4963:t=>{t.exports=function(o,e){for(var r=-1,a=o==null?0:o.length,i=0,n=[];++r<a;){var c=o[r];e(c,r,o)&&(n[i++]=c)}return n}},4636:(t,o,e)=>{var r=e(2545),a=e(5694),i=e(1469),n=e(4144),c=e(5776),p=e(6719),d=Object.prototype.hasOwnProperty;t.exports=function(m,x){var g=i(m),v=!g&&a(m),h=!g&&!v&&n(m),b=!g&&!v&&!h&&p(m),j=g||v||h||b,F=j?r(m.length,String):[],O=F.length;for(var N in m)!x&&!d.call(m,N)||j&&(N=="length"||h&&(N=="offset"||N=="parent")||b&&(N=="buffer"||N=="byteLength"||N=="byteOffset")||c(N,O))||F.push(N);return F}},2488:t=>{t.exports=function(o,e){for(var r=-1,a=e.length,i=o.length;++r<a;)o[i+r]=e[r];return o}},2908:t=>{t.exports=function(o,e){for(var r=-1,a=o==null?0:o.length;++r<a;)if(e(o[r],r,o))return!0;return!1}},8470:(t,o,e)=>{var r=e(7813);t.exports=function(a,i){for(var n=a.length;n--;)if(r(a[n][0],i))return n;return-1}},8866:(t,o,e)=>{var r=e(2488),a=e(1469);t.exports=function(i,n,c){var p=n(i);return a(i)?p:r(p,c(i))}},4239:(t,o,e)=>{var r=e(2705),a=e(9607),i=e(2333),n=r?r.toStringTag:void 0;t.exports=function(c){return c==null?c===void 0?"[object Undefined]":"[object Null]":n&&n in Object(c)?a(c):i(c)}},9454:(t,o,e)=>{var r=e(4239),a=e(7005);t.exports=function(i){return a(i)&&r(i)=="[object Arguments]"}},939:(t,o,e)=>{var r=e(2492),a=e(7005);t.exports=function i(n,c,p,d,m){return n===c||(n==null||c==null||!a(n)&&!a(c)?n!=n&&c!=c:r(n,c,p,d,i,m))}},2492:(t,o,e)=>{var r=e(6384),a=e(7114),i=e(8351),n=e(6096),c=e(4160),p=e(1469),d=e(4144),m=e(6719),x="[object Arguments]",g="[object Array]",v="[object Object]",h=Object.prototype.hasOwnProperty;t.exports=function(b,j,F,O,N,y){var E=p(b),I=p(j),U=E?g:c(b),P=I?g:c(j),C=(U=U==x?v:U)==v,V=(P=P==x?v:P)==v,q=U==P;if(q&&d(b)){if(!d(j))return!1;E=!0,C=!1}if(q&&!C)return y||(y=new r),E||m(b)?a(b,j,F,O,N,y):i(b,j,U,F,O,N,y);if(!(1&F)){var B=C&&h.call(b,"__wrapped__"),A=V&&h.call(j,"__wrapped__");if(B||A){var T=B?b.value():b,S=A?j.value():j;return y||(y=new r),N(T,S,F,O,y)}}return!!q&&(y||(y=new r),n(b,j,F,O,N,y))}},8458:(t,o,e)=>{var r=e(3560),a=e(5346),i=e(3218),n=e(346),c=/^\[object .+?Constructor\]$/,p=Function.prototype,d=Object.prototype,m=p.toString,x=d.hasOwnProperty,g=RegExp("^"+m.call(x).replace(/[\\^$.*+?()[\]{}|]/g,"\\$&").replace(/hasOwnProperty|(function).*?(?=\\\()| for .+?(?=\\\])/g,"$1.*?")+"$");t.exports=function(v){return!(!i(v)||a(v))&&(r(v)?g:c).test(n(v))}},8749:(t,o,e)=>{var r=e(4239),a=e(1780),i=e(7005),n={};n["[object Float32Array]"]=n["[object Float64Array]"]=n["[object Int8Array]"]=n["[object Int16Array]"]=n["[object Int32Array]"]=n["[object Uint8Array]"]=n["[object Uint8ClampedArray]"]=n["[object Uint16Array]"]=n["[object Uint32Array]"]=!0,n["[object Arguments]"]=n["[object Array]"]=n["[object ArrayBuffer]"]=n["[object Boolean]"]=n["[object DataView]"]=n["[object Date]"]=n["[object Error]"]=n["[object Function]"]=n["[object Map]"]=n["[object Number]"]=n["[object Object]"]=n["[object RegExp]"]=n["[object Set]"]=n["[object String]"]=n["[object WeakMap]"]=!1,t.exports=function(c){return i(c)&&a(c.length)&&!!n[r(c)]}},280:(t,o,e)=>{var r=e(5726),a=e(6916),i=Object.prototype.hasOwnProperty;t.exports=function(n){if(!r(n))return a(n);var c=[];for(var p in Object(n))i.call(n,p)&&p!="constructor"&&c.push(p);return c}},2545:t=>{t.exports=function(o,e){for(var r=-1,a=Array(o);++r<o;)a[r]=e(r);return a}},1717:t=>{t.exports=function(o){return function(e){return o(e)}}},4757:t=>{t.exports=function(o,e){return o.has(e)}},4429:(t,o,e)=>{var r=e(5639)["__core-js_shared__"];t.exports=r},7114:(t,o,e)=>{var r=e(8668),a=e(2908),i=e(4757);t.exports=function(n,c,p,d,m,x){var g=1&p,v=n.length,h=c.length;if(v!=h&&!(g&&h>v))return!1;var b=x.get(n),j=x.get(c);if(b&&j)return b==c&&j==n;var F=-1,O=!0,N=2&p?new r:void 0;for(x.set(n,c),x.set(c,n);++F<v;){var y=n[F],E=c[F];if(d)var I=g?d(E,y,F,c,n,x):d(y,E,F,n,c,x);if(I!==void 0){if(I)continue;O=!1;break}if(N){if(!a(c,function(U,P){if(!i(N,P)&&(y===U||m(y,U,p,d,x)))return N.push(P)})){O=!1;break}}else if(y!==E&&!m(y,E,p,d,x)){O=!1;break}}return x.delete(n),x.delete(c),O}},8351:(t,o,e)=>{var r=e(2705),a=e(1149),i=e(7813),n=e(7114),c=e(8776),p=e(1814),d=r?r.prototype:void 0,m=d?d.valueOf:void 0;t.exports=function(x,g,v,h,b,j,F){switch(v){case"[object DataView]":if(x.byteLength!=g.byteLength||x.byteOffset!=g.byteOffset)return!1;x=x.buffer,g=g.buffer;case"[object ArrayBuffer]":return!(x.byteLength!=g.byteLength||!j(new a(x),new a(g)));case"[object Boolean]":case"[object Date]":case"[object Number]":return i(+x,+g);case"[object Error]":return x.name==g.name&&x.message==g.message;case"[object RegExp]":case"[object String]":return x==g+"";case"[object Map]":var O=c;case"[object Set]":var N=1&h;if(O||(O=p),x.size!=g.size&&!N)return!1;var y=F.get(x);if(y)return y==g;h|=2,F.set(x,g);var E=n(O(x),O(g),h,b,j,F);return F.delete(x),E;case"[object Symbol]":if(m)return m.call(x)==m.call(g)}return!1}},6096:(t,o,e)=>{var r=e(8234),a=Object.prototype.hasOwnProperty;t.exports=function(i,n,c,p,d,m){var x=1&c,g=r(i),v=g.length;if(v!=r(n).length&&!x)return!1;for(var h=v;h--;){var b=g[h];if(!(x?b in n:a.call(n,b)))return!1}var j=m.get(i),F=m.get(n);if(j&&F)return j==n&&F==i;var O=!0;m.set(i,n),m.set(n,i);for(var N=x;++h<v;){var y=i[b=g[h]],E=n[b];if(p)var I=x?p(E,y,b,n,i,m):p(y,E,b,i,n,m);if(!(I===void 0?y===E||d(y,E,c,p,m):I)){O=!1;break}N||(N=b=="constructor")}if(O&&!N){var U=i.constructor,P=n.constructor;U==P||!("constructor"in i)||!("constructor"in n)||typeof U=="function"&&U instanceof U&&typeof P=="function"&&P instanceof P||(O=!1)}return m.delete(i),m.delete(n),O}},1957:(t,o,e)=>{var r=typeof e.g=="object"&&e.g&&e.g.Object===Object&&e.g;t.exports=r},8234:(t,o,e)=>{var r=e(8866),a=e(9551),i=e(3674);t.exports=function(n){return r(n,i,a)}},5050:(t,o,e)=>{var r=e(7019);t.exports=function(a,i){var n=a.__data__;return r(i)?n[typeof i=="string"?"string":"hash"]:n.map}},852:(t,o,e)=>{var r=e(8458),a=e(7801);t.exports=function(i,n){var c=a(i,n);return r(c)?c:void 0}},9607:(t,o,e)=>{var r=e(2705),a=Object.prototype,i=a.hasOwnProperty,n=a.toString,c=r?r.toStringTag:void 0;t.exports=function(p){var d=i.call(p,c),m=p[c];try{p[c]=void 0;var x=!0}catch{}var g=n.call(p);return x&&(d?p[c]=m:delete p[c]),g}},9551:(t,o,e)=>{var r=e(4963),a=e(479),i=Object.prototype.propertyIsEnumerable,n=Object.getOwnPropertySymbols,c=n?function(p){return p==null?[]:(p=Object(p),r(n(p),function(d){return i.call(p,d)}))}:a;t.exports=c},4160:(t,o,e)=>{var r=e(8552),a=e(7071),i=e(3818),n=e(8525),c=e(577),p=e(4239),d=e(346),m="[object Map]",x="[object Promise]",g="[object Set]",v="[object WeakMap]",h="[object DataView]",b=d(r),j=d(a),F=d(i),O=d(n),N=d(c),y=p;(r&&y(new r(new ArrayBuffer(1)))!=h||a&&y(new a)!=m||i&&y(i.resolve())!=x||n&&y(new n)!=g||c&&y(new c)!=v)&&(y=function(E){var I=p(E),U=I=="[object Object]"?E.constructor:void 0,P=U?d(U):"";if(P)switch(P){case b:return h;case j:return m;case F:return x;case O:return g;case N:return v}return I}),t.exports=y},7801:t=>{t.exports=function(o,e){return o==null?void 0:o[e]}},1789:(t,o,e)=>{var r=e(4536);t.exports=function(){this.__data__=r?r(null):{},this.size=0}},401:t=>{t.exports=function(o){var e=this.has(o)&&delete this.__data__[o];return this.size-=e?1:0,e}},7667:(t,o,e)=>{var r=e(4536),a=Object.prototype.hasOwnProperty;t.exports=function(i){var n=this.__data__;if(r){var c=n[i];return c==="__lodash_hash_undefined__"?void 0:c}return a.call(n,i)?n[i]:void 0}},1327:(t,o,e)=>{var r=e(4536),a=Object.prototype.hasOwnProperty;t.exports=function(i){var n=this.__data__;return r?n[i]!==void 0:a.call(n,i)}},1866:(t,o,e)=>{var r=e(4536);t.exports=function(a,i){var n=this.__data__;return this.size+=this.has(a)?0:1,n[a]=r&&i===void 0?"__lodash_hash_undefined__":i,this}},5776:t=>{var o=/^(?:0|[1-9]\d*)$/;t.exports=function(e,r){var a=typeof e;return!!(r=r??9007199254740991)&&(a=="number"||a!="symbol"&&o.test(e))&&e>-1&&e%1==0&&e<r}},7019:t=>{t.exports=function(o){var e=typeof o;return e=="string"||e=="number"||e=="symbol"||e=="boolean"?o!=="__proto__":o===null}},5346:(t,o,e)=>{var r,a=e(4429),i=(r=/[^.]+$/.exec(a&&a.keys&&a.keys.IE_PROTO||""))?"Symbol(src)_1."+r:"";t.exports=function(n){return!!i&&i in n}},5726:t=>{var o=Object.prototype;t.exports=function(e){var r=e&&e.constructor;return e===(typeof r=="function"&&r.prototype||o)}},7040:t=>{t.exports=function(){this.__data__=[],this.size=0}},4125:(t,o,e)=>{var r=e(8470),a=Array.prototype.splice;t.exports=function(i){var n=this.__data__,c=r(n,i);return!(c<0||(c==n.length-1?n.pop():a.call(n,c,1),--this.size,0))}},2117:(t,o,e)=>{var r=e(8470);t.exports=function(a){var i=this.__data__,n=r(i,a);return n<0?void 0:i[n][1]}},7518:(t,o,e)=>{var r=e(8470);t.exports=function(a){return r(this.__data__,a)>-1}},4705:(t,o,e)=>{var r=e(8470);t.exports=function(a,i){var n=this.__data__,c=r(n,a);return c<0?(++this.size,n.push([a,i])):n[c][1]=i,this}},4785:(t,o,e)=>{var r=e(1989),a=e(8407),i=e(7071);t.exports=function(){this.size=0,this.__data__={hash:new r,map:new(i||a),string:new r}}},1285:(t,o,e)=>{var r=e(5050);t.exports=function(a){var i=r(this,a).delete(a);return this.size-=i?1:0,i}},6e3:(t,o,e)=>{var r=e(5050);t.exports=function(a){return r(this,a).get(a)}},9916:(t,o,e)=>{var r=e(5050);t.exports=function(a){return r(this,a).has(a)}},5265:(t,o,e)=>{var r=e(5050);t.exports=function(a,i){var n=r(this,a),c=n.size;return n.set(a,i),this.size+=n.size==c?0:1,this}},8776:t=>{t.exports=function(o){var e=-1,r=Array(o.size);return o.forEach(function(a,i){r[++e]=[i,a]}),r}},4536:(t,o,e)=>{var r=e(852)(Object,"create");t.exports=r},6916:(t,o,e)=>{var r=e(5569)(Object.keys,Object);t.exports=r},1167:(t,o,e)=>{t=e.nmd(t);var r=e(1957),a=o&&!o.nodeType&&o,i=a&&t&&!t.nodeType&&t,n=i&&i.exports===a&&r.process,c=function(){try{return i&&i.require&&i.require("util").types||n&&n.binding&&n.binding("util")}catch{}}();t.exports=c},2333:t=>{var o=Object.prototype.toString;t.exports=function(e){return o.call(e)}},5569:t=>{t.exports=function(o,e){return function(r){return o(e(r))}}},5639:(t,o,e)=>{var r=e(1957),a=typeof self=="object"&&self&&self.Object===Object&&self,i=r||a||Function("return this")();t.exports=i},619:t=>{t.exports=function(o){return this.__data__.set(o,"__lodash_hash_undefined__"),this}},2385:t=>{t.exports=function(o){return this.__data__.has(o)}},1814:t=>{t.exports=function(o){var e=-1,r=Array(o.size);return o.forEach(function(a){r[++e]=a}),r}},7465:(t,o,e)=>{var r=e(8407);t.exports=function(){this.__data__=new r,this.size=0}},3779:t=>{t.exports=function(o){var e=this.__data__,r=e.delete(o);return this.size=e.size,r}},7599:t=>{t.exports=function(o){return this.__data__.get(o)}},4758:t=>{t.exports=function(o){return this.__data__.has(o)}},4309:(t,o,e)=>{var r=e(8407),a=e(7071),i=e(3369);t.exports=function(n,c){var p=this.__data__;if(p instanceof r){var d=p.__data__;if(!a||d.length<199)return d.push([n,c]),this.size=++p.size,this;p=this.__data__=new i(d)}return p.set(n,c),this.size=p.size,this}},346:t=>{var o=Function.prototype.toString;t.exports=function(e){if(e!=null){try{return o.call(e)}catch{}try{return e+""}catch{}}return""}},7813:t=>{t.exports=function(o,e){return o===e||o!=o&&e!=e}},5694:(t,o,e)=>{var r=e(9454),a=e(7005),i=Object.prototype,n=i.hasOwnProperty,c=i.propertyIsEnumerable,p=r(function(){return arguments}())?r:function(d){return a(d)&&n.call(d,"callee")&&!c.call(d,"callee")};t.exports=p},1469:t=>{var o=Array.isArray;t.exports=o},8612:(t,o,e)=>{var r=e(3560),a=e(1780);t.exports=function(i){return i!=null&&a(i.length)&&!r(i)}},4144:(t,o,e)=>{t=e.nmd(t);var r=e(5639),a=e(5062),i=o&&!o.nodeType&&o,n=i&&t&&!t.nodeType&&t,c=n&&n.exports===i?r.Buffer:void 0,p=(c?c.isBuffer:void 0)||a;t.exports=p},8446:(t,o,e)=>{var r=e(939);t.exports=function(a,i){return r(a,i)}},3560:(t,o,e)=>{var r=e(4239),a=e(3218);t.exports=function(i){if(!a(i))return!1;var n=r(i);return n=="[object Function]"||n=="[object GeneratorFunction]"||n=="[object AsyncFunction]"||n=="[object Proxy]"}},1780:t=>{t.exports=function(o){return typeof o=="number"&&o>-1&&o%1==0&&o<=9007199254740991}},3218:t=>{t.exports=function(o){var e=typeof o;return o!=null&&(e=="object"||e=="function")}},7005:t=>{t.exports=function(o){return o!=null&&typeof o=="object"}},6719:(t,o,e)=>{var r=e(8749),a=e(1717),i=e(1167),n=i&&i.isTypedArray,c=n?a(n):r;t.exports=c},3674:(t,o,e)=>{var r=e(4636),a=e(280),i=e(8612);t.exports=function(n){return i(n)?r(n):a(n)}},479:t=>{t.exports=function(){return[]}},5062:t=>{t.exports=function(){return!1}},75:function(t){(function(){var o,e,r,a,i,n;typeof performance<"u"&&performance!==null&&performance.now?t.exports=function(){return performance.now()}:typeof process<"u"&&process!==null&&process.hrtime?(t.exports=function(){return(o()-i)/1e6},e=process.hrtime,a=(o=function(){var c;return 1e9*(c=e())[0]+c[1]})(),n=1e9*process.uptime(),i=a-n):Date.now?(t.exports=function(){return Date.now()-r},r=Date.now()):(t.exports=function(){return new Date().getTime()-r},r=new Date().getTime())}).call(this)},4087:(t,o,e)=>{for(var r=e(75),a=typeof window>"u"?e.g:window,i=["moz","webkit"],n="AnimationFrame",c=a["request"+n],p=a["cancel"+n]||a["cancelRequest"+n],d=0;!c&&d<i.length;d++)c=a[i[d]+"Request"+n],p=a[i[d]+"Cancel"+n]||a[i[d]+"CancelRequest"+n];if(!c||!p){var m=0,x=0,g=[];c=function(v){if(g.length===0){var h=r(),b=Math.max(0,16.666666666666668-(h-m));m=b+h,setTimeout(function(){var j=g.slice(0);g.length=0;for(var F=0;F<j.length;F++)if(!j[F].cancelled)try{j[F].callback(m)}catch(O){setTimeout(function(){throw O},0)}},Math.round(b))}return g.push({handle:++x,callback:v,cancelled:!1}),x},p=function(v){for(var h=0;h<g.length;h++)g[h].handle===v&&(g[h].cancelled=!0)}}t.exports=function(v){return c.call(a,v)},t.exports.cancel=function(){p.apply(a,arguments)},t.exports.polyfill=function(v){v||(v=a),v.requestAnimationFrame=c,v.cancelAnimationFrame=p}},8156:t=>{t.exports=D}},de={};function k(t){var o=de[t];if(o!==void 0)return o.exports;var e=de[t]={id:t,loaded:!1,exports:{}};return te[t].call(e.exports,e,e.exports,k),e.loaded=!0,e.exports}k.n=t=>{var o=t&&t.__esModule?()=>t.default:()=>t;return k.d(o,{a:o}),o},k.d=(t,o)=>{for(var e in o)k.o(o,e)&&!k.o(t,e)&&Object.defineProperty(t,e,{enumerable:!0,get:o[e]})},k.g=function(){if(typeof globalThis=="object")return globalThis;try{return this||new Function("return this")()}catch{if(typeof window=="object")return window}}(),k.o=(t,o)=>Object.prototype.hasOwnProperty.call(t,o),k.nmd=t=>(t.paths=[],t.children||(t.children=[]),t);var ue={};return(()=>{k.d(ue,{default:()=>g});var t=k(8156),o=k.n(t),e=k(7403),r=k(8446),a=k.n(r);function i(v){return i=typeof Symbol=="function"&&typeof Symbol.iterator=="symbol"?function(h){return typeof h}:function(h){return h&&typeof Symbol=="function"&&h.constructor===Symbol&&h!==Symbol.prototype?"symbol":typeof h},i(v)}function n(v,h){for(var b=0;b<h.length;b++){var j=h[b];j.enumerable=j.enumerable||!1,j.configurable=!0,"value"in j&&(j.writable=!0),Object.defineProperty(v,m(j.key),j)}}function c(v,h){return c=Object.setPrototypeOf?Object.setPrototypeOf.bind():function(b,j){return b.__proto__=j,b},c(v,h)}function p(v){if(v===void 0)throw new ReferenceError("this hasn't been initialised - super() hasn't been called");return v}function d(v){return d=Object.setPrototypeOf?Object.getPrototypeOf.bind():function(h){return h.__proto__||Object.getPrototypeOf(h)},d(v)}function m(v){var h=function(b,j){if(i(b)!=="object"||b===null)return b;var F=b[Symbol.toPrimitive];if(F!==void 0){var O=F.call(b,"string");if(i(O)!=="object")return O;throw new TypeError("@@toPrimitive must return a primitive value.")}return String(b)}(v);return i(h)==="symbol"?h:String(h)}var x=function(v){(function(y,E){if(typeof E!="function"&&E!==null)throw new TypeError("Super expression must either be null or a function");y.prototype=Object.create(E&&E.prototype,{constructor:{value:y,writable:!0,configurable:!0}}),Object.defineProperty(y,"prototype",{writable:!1}),E&&c(y,E)})(N,v);var h,b,j,F,O=(j=N,F=function(){if(typeof Reflect>"u"||!Reflect.construct||Reflect.construct.sham)return!1;if(typeof Proxy=="function")return!0;try{return Boolean.prototype.valueOf.call(Reflect.construct(Boolean,[],function(){})),!0}catch{return!1}}(),function(){var y,E=d(j);if(F){var I=d(this).constructor;y=Reflect.construct(E,arguments,I)}else y=E.apply(this,arguments);return function(U,P){if(P&&(i(P)==="object"||typeof P=="function"))return P;if(P!==void 0)throw new TypeError("Derived constructors may only return object or undefined");return p(U)}(this,y)});function N(){var y,E,I,U;(function(q,B){if(!(q instanceof B))throw new TypeError("Cannot call a class as a function")})(this,N);for(var P=arguments.length,C=new Array(P),V=0;V<P;V++)C[V]=arguments[V];return E=p(y=O.call.apply(O,[this].concat(C))),U={instance:null},(I=m(I="state"))in E?Object.defineProperty(E,I,{value:U,enumerable:!0,configurable:!0,writable:!0}):E[I]=U,y}return h=N,(b=[{key:"componentDidMount",value:function(){var y=this,E=new e.default(this.typewriter,this.props.options);this.setState({instance:E},function(){var I=y.props.onInit;I&&I(E)})}},{key:"componentDidUpdate",value:function(y){a()(this.props.options,y.options)||this.setState({instance:new e.default(this.typewriter,this.props.options)})}},{key:"componentWillUnmount",value:function(){this.state.instance&&this.state.instance.stop()}},{key:"render",value:function(){var y=this,E=this.props.component;return o().createElement(E,{ref:function(I){return y.typewriter=I},className:"Typewriter","data-testid":"typewriter-wrapper"})}}])&&n(h.prototype,b),Object.defineProperty(h,"prototype",{writable:!1}),N}(t.Component);x.defaultProps={component:"div"};const g=x})(),ue.default})())})(ye);var De=ye.exports;const Ke=Oe(De),Qe="data:image/png;base64,",ze="data:image/png;base64,",Ve="data:image/png;base64,",qe="/assets/graphic-2@2x-o9q-Ivx4.png",Xe="data:image/png;base64,",He="/assets/graphic-3@2x-DFvq1ZvH.png",Ze="/assets/graphic-4-DEP80msl.png",_e="/assets/graphic-4@2x-BSxi8HUF.png",$e="/assets/main-graphic-BH5TrvAQ.png",et="/assets/main-graphic@2x-DrmctVov.png",Ae="/assets/open-source-u8EY6AMF.png",ve="/assets/open-source@2x-q94tbrLH.png",tt="data:image/png;base64,",rt="data:image/png;base64,",nt="data:image/png;base64,",ot="/assets/step-1@2x-BnC6LKQH.png",it="data:image/png;base64,",at="/assets/step-2@2x-Cv5N9IX7.png",st="data:image/png;base64,",ct="/assets/step-3@2x-0j47Yci1.png",lt="data:image/png;base64,",pt="data:image/png;base64,",le=ke("#1683fb","#1683fb","#ffffff","#0576f1","#0576f1","#ffffff"),J={smMaxWidth:767,mdMinWidth:768,mdMaxWidth:1048,lgMinWidth:1049,lgMaxWidth:1365,xlMinWidth:1366},f={sm:`@media (max-width: ${J.smMaxWidth}px)`,smmd:`@media (max-width: ${J.mdMaxWidth}px)`,md:`@media (min-width: ${J.mdMinWidth}px) and (max-width: ${J.mdMaxWidth}px)`,mdlg:`@media (min-width: ${J.mdMinWidth}px) and (max-width: ${J.lgMaxWidth}px)`,lg:`@media (min-width: ${J.lgMinWidth}px) and (max-width: ${J.lgMaxWidth}px)`,lgxl:`@media (min-width: ${J.lgMinWidth}px)`,xl:`@media (min-width: ${J.xlMinWidth}px)`},dt=pe.memo(()=>{const L={strings:["private files.","confidentials.","secrets."],autoStart:!0,loop:!0,delay:70,deleteSpeed:50,pauseFor:1500};return l.jsx("span",{className:u`
        & .Typewriter {
          display: inline;
        }

        & .Typewriter__cursor {
          color: #ffd15c;
          font-weight: 300;
          margin-left: -5px;
          position: relative;
          bottom: 9px;
          font-size: inherit;
        }
      `,children:l.jsx(Ke,{options:L})})}),vt=pe.memo(()=>{Le();const K=Ce();return l.jsxs("div",{className:u`
        display: flex;
        flex-direction: column;
        align-items: center;
      `,children:[l.jsxs("div",{className:u`
          width: 100%;
          padding-top: 20px;
          padding-bottom: 20px;
          display: flex;
          flex-direction: row;
          align-items: center;
          justify-content: space-between;
          margin-bottom: 30px;

          ${f.sm} {
            padding-left: 15px;
            padding-right: 15px;
            padding-top: 7px;
            padding-bottom: 15px;
          }

          ${f.md} {
            padding-top: 20px;
          }

          ${f.mdlg} {
            padding-left: 28px;
            padding-right: 28px;
          }

          ${f.xl} {
            width: 1280px;
          }
        `,children:[l.jsx(Ye,{}),l.jsx("div",{className:u`
            display: flex;
            align-items: center;
          `,children:l.jsx(se,{href:"/login",className:ae(le,u`
                font-size: 16px;
                padding: 12px 19px;
                font-weight: 700;
                width: 131px;
                line-height: 22px;

                ${f.sm} {
                  font-size: 14px;
                  padding: 7px 6px;
                  width: inherit;
                  line-height: 22px;
                  width: 103px;
                }

                ${f.md} {
                  line-height: 12px;
                }
              `),children:"Get started"})})]}),l.jsxs("div",{className:u`
          display: flex;
          flex-direction: row;
          margin-bottom: 70px;

          ${f.sm} {
            padding-left: 15px;
            padding-right: 15px;
          }

          ${f.smmd} {
            flex-direction: column;
            max-width: 556px;
          }

          ${f.mdlg} {
            padding-left: 28px;
            padding-right: 28px;
          }

          ${f.lg} {
            width: 100%;
          }

          ${f.xl} {
            width: 1280px;
          }
        `,children:[l.jsxs("div",{className:u`
            display: flex;
            flex-direction: column;
            flex-grow: 1;
          `,children:[l.jsxs("h1",{className:u`
              font-size: 64px;
              line-height: 1.08;
              font-weight: 700;
              color: #011722;
              margin: 0 0 36px;

              ${f.sm} {
                font-size: 51px;
                margin: 0 0 26px;
              }

              ${f.md} {
                width: 700px;
              }

              @media (max-width: 548px) {
                font-size: 45px;
              }

              @media (max-width: 482px) {
                font-size: 43px;
              }

              @media (max-width: 450px) {
                font-size: 38px;
              }

              @media (max-width: 420px) {
                font-size: 28px;
              }
            `,children:["One vault for all",l.jsx("br",{}),l.jsxs("span",{className:u`
                font-weight: 800;
              `,children:["your ",l.jsx(dt,{})]})]}),l.jsx("p",{className:u`
              font-size: 18px;
              color: #011722;
              margin: 0 0 38px;

              ${f.sm} {
                font-size: 14px;
                margin: 0 0 26px;
              }

              ${f.lgxl} {
                width: 565px;
              }
            `,children:"Powerful, open source, client-side, zero-knowledge encryption. Unlock enhanced security for your most sensitive files with Koofr Vault."}),l.jsx(se,{href:"/login",className:ae(le,u`
                font-size: 18px;
                padding: 9px 21px;
                font-weight: 700;

                ${f.smmd} {
                  width: 100%;
                }

                ${f.lgxl} {
                  width: 500px;
                }
              `),children:"Get started"}),K.appStoreUrl!==void 0||K.googlePlayUrl!==void 0||K.fDroidUrl!==void 0?l.jsxs("div",{className:u`
                display: flex;
                flex-direction: row;
                flex-wrap: wrap;
                gap: 20px;
                margin-top: 32px;

                ${f.smmd} {
                  justify-content: center;
                }
              `,children:[K.googlePlayUrl!==void 0?l.jsx("a",{href:K.googlePlayUrl,target:"_blank",rel:"noreferrer",children:l.jsx(G,{image:Ie,image2x:Ue,width:122,height:36})}):null,K.appStoreUrl!==void 0?l.jsx("a",{href:K.appStoreUrl,target:"_blank",rel:"noreferrer",children:l.jsx(G,{image:Pe,image2x:Me,width:117,height:36})}):null,K.fDroidUrl!==void 0?l.jsx("a",{href:K.fDroidUrl,target:"_blank",rel:"noreferrer",children:l.jsx(G,{image:Re,image2x:We,width:123,height:36})}):null]}):null]}),l.jsx("div",{className:u`
            ${f.smmd} {
              display: flex;
              flex-direction: column;
              align-items: center;
              padding: 38px 0;
            }

            ${f.lgxl} {
              padding: 25px 33px 0;
              flex-shrink: 0;
            }
          `,children:l.jsx(G,{image:$e,image2x:et,width:480,height:338,classNameForSize:(L,D)=>u`
              ${f.sm} {
                background-size: 290px ${290*D/L}px;
                width: 290px;
                height: ${290*D/L}px;
              }

              @media (min-width: 1000px) and (max-width: 1049px) {
                background-size: ${L*.6}px ${D*.6}px;
                width: ${L*.6}px;
                height: ${D*.6}px;
              }

              @media (min-width: 1050px) and (max-width: 1169px) {
                background-size: ${L*.7}px ${D*.7}px;
                width: ${L*.7}px;
                height: ${D*.7}px;
              }
              @media (min-width: 1170px) and (max-width: 1271px) {
                background-size: ${L*.8}px ${D*.8}px;
                width: ${L*.8}px;
                height: ${D*.8}px;
              }
            `})})]}),l.jsxs("div",{className:u`
          display: flex;
          flex-direction: column;
          margin-bottom: 107px;

          ${f.sm} {
            padding-left: 15px;
            padding-right: 15px;
          }

          ${f.smmd} {
            max-width: 556px;
          }

          ${f.mdlg} {
            padding-left: 28px;
            padding-right: 28px;
          }

          ${f.lg} {
            width: 100%;
          }

          ${f.xl} {
            width: 1280px;
          }
        `,children:[l.jsx("h2",{className:u`
            font-size: 30px;
            font-weight: 700;
            color: #011722;
            margin: 0 0 23px;
          `,children:"Extra strong protection"}),l.jsxs("p",{className:u`
            font-size: 18px;
            color: #011722;

            ${f.smmd} {
              margin: 0 0 50px;
            }

            ${f.lgxl} {
              width: 500px;
              margin: 0 0 32px;
            }
          `,children:["Koofr Vault is an open source, client-side, zero-knowledge encrypted storage application by"," ",l.jsx("a",{href:"https://koofr.eu",target:"_blank",rel:"noopener",children:"Koofr cloud storage"}),". It's like having a box with a unique lock in your trunk: an extra layer of security to protect your files."]}),l.jsxs("div",{className:u`
            display: flex;

            ${f.smmd} {
              flex-direction: column;
              align-items: center;
            }

            ${f.lgxl} {
              flex-direction: row;
              justify-content: space-between;
            }
          `,children:[l.jsxs("div",{className:u`
              display: flex;
              flex-direction: column;
              width: 286px;

              @media (min-width: 1000px) and (max-width: 1069px) {
                width: 250px;
              }

              @media (min-width: 1070px) and (max-width: 1170px) {
                width: 250px;
              }
            `,children:[l.jsx("div",{className:u`
                height: 150px;
                display: flex;
                flex-direction: column;
                justify-content: center;
                align-items: center;
                margin-bottom: 22px;
              `,children:l.jsx(G,{image:nt,image2x:ot,width:70,height:86})}),l.jsx("h3",{className:u`
                font-size: 18px;
                font-weight: 600;
                color: #011722;
                margin: 0 0 10px;
              `,children:"Private Safe Key"}),l.jsx("p",{className:u`
                font-size: 18px;
                color: #011722;
                margin: 0;
              `,children:"Choose a Safe Key to create a Safe Box in your Koofr Vault."})]}),l.jsxs("div",{className:u`
              display: flex;
              flex-direction: column;
              width: 286px;

              @media (min-width: 1000px) and (max-width: 1069px) {
                width: 250px;
              }

              @media (min-width: 1070px) and (max-width: 1170px) {
                width: 250px;
              }
            `,children:[l.jsx("div",{className:u`
                height: 150px;
                display: flex;
                flex-direction: column;
                justify-content: center;
                align-items: center;
                margin-bottom: 22px;
              `,children:l.jsx(G,{image:it,image2x:at,width:70,height:86})}),l.jsx("h3",{className:u`
                font-size: 18px;
                font-weight: 600;
                color: #011722;
                margin: 0 0 10px;
              `,children:"It all stays in your browser"}),l.jsx("p",{className:u`
                font-size: 18px;
                color: #011722;
                margin: 0;
              `,children:"Your Safe Box files are encrypted or decrypted on demand, on your device."})]}),l.jsxs("div",{className:u`
              display: flex;
              flex-direction: column;
              width: 286px;

              @media (min-width: ${J.lgMinWidth}px) and (max-width: 1069px) {
                width: 210px;
              }

              @media (min-width: 1070px) and (max-width: 1170px) {
                width: 250px;
              }
            `,children:[l.jsx("div",{className:u`
                height: 150px;
                display: flex;
                flex-direction: column;
                justify-content: center;
                align-items: center;
                margin-bottom: 22px;
              `,children:l.jsx(G,{image:st,image2x:ct,width:70,height:86})}),l.jsx("h3",{className:u`
                font-size: 18px;
                font-weight: 600;
                color: #011722;
                margin: 0 0 10px;
              `,children:"Only you have access"}),l.jsx("p",{className:u`
                font-size: 18px;
                color: #011722;
                margin: 0;
              `,children:"Cloud content is encrypted and never accessible without your Safe Key."})]})]})]}),l.jsxs("div",{className:u`
          width: 100%;
          display: flex;

          ${f.sm} {
            padding-left: 15px;
            padding-right: 15px;
          }

          ${f.smmd} {
            flex-direction: column-reverse;
            align-items: center;
            margin-bottom: 120px;
          }

          ${f.mdlg} {
            padding-left: 28px;
            padding-right: 28px;
          }

          ${f.lgxl} {
            flex-direction: row;
            align-items: center;
            margin-bottom: 120px;
          }

          ${f.xl} {
            width: 1280px;
          }
        `,children:[l.jsx("div",{className:u`
            width: 60%;
            display: flex;
            flex-direction: column;
            align-items: center;
            flex-grow: 1;
          `,children:l.jsx(G,{image:Ze,image2x:_e,width:356,height:414,classNameForSize:(L,D)=>u`
              ${f.sm} {
                background-size: 290px ${290*D/L}px;
                width: 290px;
                height: ${290*D/L}px;
              }
            `})}),l.jsxs("div",{className:u`
            display: flex;
            flex-direction: column;
            max-width: 500px;
            flex-shrink: 0;
          `,children:[l.jsx("h2",{className:u`
              font-size: 30px;
              font-weight: 700;
              color: #011722;
              margin: 0 0 22px;
            `,children:"For your eyes only"}),l.jsx("p",{className:u`
              font-size: 18px;
              color: #011722;
              margin: 0 0 32px;
            `,children:"Your files are encrypted, file names and all content included, locally on your device with your Safe Key and some magic salt before they are sent to your Vault."}),l.jsxs("p",{className:u`
              font-size: 18px;
              color: #011722;
              margin: 0 0 32px;

              ${f.smmd} {
                margin: 0 0 70px;
              }
            `,children:["Neither your Safe key nor any other unencrypted file data or metadata is sent to or stored by"," ",l.jsx("a",{href:"https://koofr.eu",target:"_blank",rel:"noopener",children:"Koofr"}),". Only you can decrypt and access your Vault files."]})]})]}),l.jsx("div",{className:u`
          width: 100%;
          display: flex;
          flex-direction: column;
          align-items: center;
          background-color: #011722;

          ${f.smmd} {
            padding: 60px 0 60px;
          }

          ${f.lgxl} {
            padding: 111px 0 120px;
          }
        `,children:l.jsxs("div",{className:u`
            display: flex;

            ${f.sm} {
              padding-left: 15px;
              padding-right: 15px;
            }

            ${f.smmd} {
              flex-direction: column;
              max-width: 556px;
            }

            ${f.mdlg} {
              padding-left: 28px;
              padding-right: 28px;
            }

            ${f.lg} {
              width: 100%;
            }

            ${f.lgxl} {
              flex-direction: row;
              justify-content: space-between;
            }

            ${f.xl} {
              width: 1280px;
            }
          `,children:[l.jsx("div",{className:u`
              display: flex;
              flex-direction: column;
              flex-grow: 1;

              ${f.smmd} {
                align-items: center;
                margin-bottom: 40px;
              }

              ${f.lgxl} {
                margin-top: 71px;
              }

              @media (min-width: ${J.lgMinWidth}px) and (max-width: 1249px) {
                display: none;
              }
            `,children:l.jsx("a",{href:"https://github.com/koofr/vault",target:"_blank",rel:"noreferrer",children:l.jsx(G,{image:Ae,image2x:ve,width:308,height:150})})}),l.jsxs("div",{className:u`
              display: flex;
              flex-direction: column;
              flex-shrink: 0;

              ${f.lgxl} {
                width: 500px;
                margin: 0 40px;
              }
            `,children:[l.jsx("h2",{className:u`
                font-size: 30px;
                font-weight: 700;
                color: #f4f5f5;
                margin: 0 0 22px;
              `,children:"Fully open source"}),l.jsxs("p",{className:u`
                font-size: 18px;
                color: #f4f5f5;
                margin: 0 0 32px;
              `,children:["Koofr Vault is"," ",l.jsx("a",{href:"https://github.com/koofr/vault",target:"_blank",rel:"noreferrer",className:u`
                  ${ce} {
                    color: #f4f5f5;
                    text-decoration: underline;
                  }
                `,children:"open source"}),", so you can always check that the code does exactly what is promised - and nothing more. File encryption is performed using NaCl SecretBox, which uses XSalsa20 cipher and Poly1305 for ensuring integrity."]}),l.jsxs("p",{className:u`
                font-size: 18px;
                color: #f4f5f5;

                ${f.smmd} {
                  margin: 0 0 50px;
                }

                ${f.lgxl} {
                  margin: 0 0 78px;
                }

                @media (min-width: ${J.lgMinWidth}px) and (max-width: 1249px) {
                  margin: 0 0 0;
                }
              `,children:["It is compatible with"," ",l.jsx("a",{href:"https://rclone.org",target:"_blank",rel:"noreferrer",className:u`
                  ${ce} {
                    color: #f4f5f5;
                    text-decoration: underline;
                  }
                `,children:"rclone"}),". This means that you can download your encrypted files and decrypt them locally using the rclone command-line tool."]})]}),l.jsxs("div",{className:u`
              display: flex;
              flex-direction: column;

              ${f.smmd} {
                align-items: center;
              }

              ${f.lgxl} {
                justify-content: flex-end;
                align-items: flex-end;
                flex-grow: 1;
              }

              @media (min-width: ${J.lgMinWidth}px) and (max-width: 1249px) {
                align-items: center;
              }
            `,children:[l.jsx("div",{className:u`
                display: none;

                @media (min-width: ${J.lgMinWidth}px) and (max-width: 1249px) {
                  display: flex;
                  margin: 0 22px 30px 0;
                }
              `,children:l.jsx("a",{href:"https://github.com/koofr/vault",target:"_blank",rel:"noreferrer",children:l.jsx(G,{image:Ae,image2x:ve,width:308,height:150})})}),l.jsxs("div",{className:u`
                display: flex;
                flex-direction: row;
                justify-content: center;
                align-items: center;
                border: 1px solid #565656;
                width: 288px;
                height: 152px;
              `,children:[l.jsx("a",{href:"https://github.com/koofr/vault",target:"_blank",rel:"noreferrer",className:u`
                  margin: 0 23px;
                `,children:l.jsx(G,{image:lt,image2x:pt,width:54,height:54})}),l.jsx("a",{href:"https://rclone.org/",target:"_blank",rel:"noreferrer",className:u`
                  margin: 0 23px;
                `,children:l.jsx(G,{image:tt,image2x:rt,width:55,height:52})})]})]})]})}),l.jsxs("div",{className:u`
          display: flex;

          ${f.sm} {
            padding-left: 15px;
            padding-right: 15px;
          }

          ${f.smmd} {
            flex-direction: column;
            max-width: 556px;
            padding-top: 60px;
            padding-bottom: 60px;
          }

          ${f.mdlg} {
            padding-left: 28px;
            padding-right: 28px;
          }

          ${f.lg} {
            width: 100%;
          }

          ${f.lgxl} {
            flex-direction: row;
            justify-content: center;
            padding-top: 76px;
            padding-bottom: 76px;
          }

          ${f.xl} {
            width: 1280px;
          }
        `,children:[l.jsxs("div",{className:u`
            display: flex;
            flex-direction: column;
            align-items: center;

            ${f.smmd} {
              margin-bottom: 50px;
            }

            ${f.lgxl} {
              width: 350px;
            }
          `,children:[l.jsx("div",{className:u`
              width: 150px;
              height: 112px;
              border: 1px solid #d4d6d7;
              display: flex;
              flex-direction: column;
              justify-content: center;
              align-items: center;
              margin-bottom: 15px;
            `,children:l.jsx(G,{image:Qe,image2x:ze,width:70,height:61})}),l.jsx("p",{className:u`
              font-size: 18px;
              font-weight: 600;
              color: #011722;
              margin: 0;
              text-align: center;
            `,children:"Client-side encryption"})]}),l.jsxs("div",{className:u`
            display: flex;
            flex-direction: column;
            align-items: center;

            ${f.smmd} {
              margin-bottom: 50px;
            }

            ${f.lgxl} {
              width: 350px;
            }
          `,children:[l.jsx("div",{className:u`
              width: 150px;
              height: 112px;
              border: 1px solid #d4d6d7;
              display: flex;
              flex-direction: column;
              justify-content: center;
              align-items: center;
              margin-bottom: 15px;
            `,children:l.jsx(G,{image:Ve,image2x:qe,width:70,height:61})}),l.jsx("p",{className:u`
              font-size: 18px;
              font-weight: 600;
              color: #011722;
              margin: 0;
              text-align: center;
            `,children:"Verifiable source code"})]}),l.jsxs("div",{className:u`
            display: flex;
            flex-direction: column;
            align-items: center;

            ${f.smmd} {
              margin-bottom: 0;
            }

            ${f.lgxl} {
              width: 350px;
            }
          `,children:[l.jsx("div",{className:u`
              width: 150px;
              height: 112px;
              border: 1px solid #d4d6d7;
              display: flex;
              flex-direction: column;
              justify-content: center;
              align-items: center;
              margin-bottom: 15px;
            `,children:l.jsx(G,{image:Xe,image2x:He,width:70,height:61})}),l.jsx("p",{className:u`
              font-size: 18px;
              font-weight: 600;
              color: #011722;
              margin: 0;
              text-align: center;
            `,children:"Zero knowledge"})]})]}),l.jsx("div",{className:u`
          width: 100%;
          border-bottom: 1px solid #d4d6d7;

          ${f.smmd} {
            margin-bottom: 60px;
          }

          ${f.lgxl} {
            margin-bottom: 111px;
          }
        `}),l.jsxs("div",{className:u`
          display: flex;
          flex-direction: column;
          align-items: center;

          ${f.sm} {
            padding-left: 15px;
            padding-right: 15px;
          }

          ${f.smmd} {
            margin: 0 0 60px;
          }

          ${f.lgxl} {
            margin: 0 0 111px;
          }
        `,children:[l.jsxs("div",{className:u`
            display: flex;
            flex-direction: column;
            max-width: 500px;
          `,children:[l.jsx("h2",{className:u`
              font-size: 30px;
              font-weight: 700;
              color: #011722;
              flex-grow: 1;
              margin: 0 0 22px;
              text-align: center;
            `,children:"Unlock your Vault"}),l.jsxs("p",{className:u`
              font-size: 18px;
              color: #011722;
              margin: 0 0 43px;
              text-align: center;
            `,children:["Start encrypting your cloud storage files in just a few minutes. Included in all"," ",l.jsx("a",{href:"https://koofr.eu/pricing",target:"_blank",rel:"noopener",children:"Koofr plans"}),"."]}),l.jsx(se,{href:"/login",className:ae(le,u`
                width: 100%;
                font-size: 18px;
                padding: 9px 21px;
                font-weight: 700;
              `),children:"Get started"})]}),l.jsxs("p",{className:u`
            font-size: 18px;
            color: #011722;
            margin: 43px 0 0;
            text-align: center;
          `,children:["Have questions about Koofr Vault?"," ",l.jsx("a",{href:"https://koofr.eu/help/koofr-vault",target:"_blank",rel:"noopener",children:"Find answers here"}),"."]})]}),l.jsx("div",{className:u`
          width: 100%;
          border-bottom: 1px solid #d4d6d7;
        `}),l.jsxs("div",{className:u`
          ${f.smmd} {
            padding: 30px 28px;
          }

          ${f.lgxl} {
            padding: 40px 28px;
          }
        `,children:[l.jsxs("p",{className:u`
            font-size: 14px;
            color: #011722;
            text-align: center;
            margin: 0 0 15px;
          `,children:["© 2024."," ",l.jsx("a",{href:"https://koofr.eu",target:"_blank",rel:"noopener",className:u`
              font-weight: 600;
              ${ce} {
                color: #011722;
                text-decoration: none;
              }
            `,children:"Koofr d.o.o."})," ","all rights reserved."]}),l.jsxs("div",{className:u`
            font-size: 13px;
            color: #011722;
            text-align: center;
          `,children:[l.jsx(Ge,{}),l.jsx(Je,{})]})]})]})});export{vt as LandingPageOfficial};
